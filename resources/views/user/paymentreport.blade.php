...
<head>
  <meta charset="utf-8">
  <title>Receipt {{$shipments->payment_id}}</title>
  <style>
    @include('PDF.style')
  </style>
</head>
...
<header class="clearfix">
  ...
  <h3>Receipt {{$shipments->payment_id}}</h3>
...
<div id="project" >
  <div style="font-size: 15px;">Paid By: {{$shipments->name}}</div><br>
 ...
 <span style="color: green; border:1px solid green; border-radius: 10% ;width: 200px;
	height: 200px; font-size: 20px; ">PAID FOR</span>
  
        
        <div style="font-size: 15px;">Order ID: {{$shipments->order_id}}</div><br>
        <div style="font-size: 15px;">Payment ID: {{$shipments->payment_id}}</div>
        <div style="font-size: 15px;">Paid at: {{$shipments->created_at}}</div>
      ...
      <table style="padding-top: 100px;">
        ...
        @endforeach
          ...
          <tr>
            <td colspan="5" class="grand total" style="text-align: right;">GRAND TOTAL PAID</td>
